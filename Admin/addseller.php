...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $seller_id = $_POST['srId'];
    $seller_name = $_POST['seName'];
    ...
    $check_sql = "SELECT * FROM sellers WHERE seller_id = ?";
    $check_stmt = $conn->prepare($check_sql);
    $check_stmt->bind_param("s", $seller_id);
    $check_stmt->execute();
    $result = $check_stmt->get_result();

    if ($result->num_rows > 0) {
        echo "<script>alert('Seller with ID $seller_id already exists!');</script>";
    } else {
        $sql = "INSERT INTO sellers (seller_id, seller_name, address, address2, city, state, zip) 
            VALUES (?, ?, ?, ?, ?, ?, ?)";
    
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssss", $seller_id, $seller_name, $address, $address2, $city, $state, $zip);
                            ...
                            <div class="white_card_body">
                                <div class="card-body">
                                    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                                        <div class="row mb-3">
                                            <div class="col-md-6">
                                                <label class="form-label" for="srId">Seller Id</label>
                                                <input type="text" class="form-control" id="srId" name="srId" placeholder="Seller Id" required />
                                            </div>
                                            <div class="col-md-6">
                                                <label class="form-label" for="seName">Seller Name</label>
                                                <input type="text" class="form-control" id="seName" name="seName" placeholder="Seller Name" required />
                                            </div>
                                        </div>
